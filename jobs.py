    ...
    if enabled == 1:
        ...
        if channels_df.shape[0] > 0:
            ...
            if channels_df.shape[0] > 0:
                ...
                channels_df['rebal_ppm'] = channels_df.apply(lambda row: int(((row['costs_7day']-row['revenue_assist_7day'])/row['amt_rebal_in_7day'])*1000000) if row['amt_rebal_in_7day'] > 0 else (row['local_fee_rate']*(row['ar_max_cost']/100) if row['revenue_assist_7day'] == 0 else 0), axis=1)
                ...
                channels_df['adjusted_out_rate'] = channels_df.apply(lambda row: int(min(row['out_rate'],row['local_fee_rate'])*(1+row['net_routed_7day']*row['assisted_ratio']*multiplier/100)), axis=1)
                ...
                channels_df['new_rate'] = channels_df.apply(lambda row: row['adjusted_out_rate'] if row['net_routed_7day'] > 0 and row['out_percent'] > down_level else (row['adjusted_rebal_rate'] if row['rebal_ppm'] > 0 and row['out_rate'] > 0 and row['out_percent'] > down_level else (row['out_rate_only'] if row['out_rate'] > 0 else (row['min_suggestion'] if row['net_routed_7day'] <= 0 and row['in_percent'] < down_level else row['fee_rate_only']))), axis=1)
                ...
                channels_df['new_rate'] = channels_df.apply(lambda row: row['local_fee_rate']*(1-limit_change) if row['adjustment']==0 and row['out_percent']>=down_level and row['net_routed_7day']==0 else row['new_rate'], axis=1)
                channels_df['new_rate'] = channels_df.apply(lambda row: row['local_fee_rate']*(1+limit_change) if row['adjustment']==0 and row['out_percent']<down_level and row['failed_out_1day']>failed_htlc_limit else row['new_rate'], axis=1)
                ...
                if not update_df.empty:
                    print (update_df[['chan_id', 'alias', 'in_percent', 'out_percent', 'net_routed_7day', 'revenue_7day', 'revenue_assist_7day', 'local_fee_rate', 'out_rate', 'rebal_ppm', 'profit_margin', 'max_suggestion', 'min_suggestion', 'assisted_ratio', 'adjusted_out_rate', 'adjusted_rebal_rate', 'out_rate_only', 'fee_rate_only', 'new_rate', 'adjustment']].to_string())
