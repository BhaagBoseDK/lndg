    ...
    if len(inactive_peers) > 0:
        ...
        for inactive_peer in inactive_peers:
            if peers.filter(pubkey=inactive_peer).exists():
                ...
                if peer.last_reconnected == None or (int((datetime.now() - peer.last_reconnected).total_seconds() / 60) > 420):
    ...
    if enabled == 1:
        ...
        if channels_df.shape[0] > 0:
            ...
            if channels_df.shape[0] > 0:
                ...
                #print (f"{datetime.now().strftime('%c')} : {forwards_df_in_7d_sum=}")
                #print (f"{datetime.now().strftime('%c')} : {forwards_df_out_7d_sum=}")
                ...
                channels_df['amt_routed_in_7day'] = channels_df.apply(lambda row: int(forwards_df_in_7d_sum.loc[row.chan_id].amt_in_msat/1000) if (forwards_df_in_7d_sum.index == row.chan_id).any() else 0, axis=1)
                ...
                payments = Payments.objects.filter(status=2).filter(creation_date__gte=filter_7day).filter(rebal_chan__isnull=False).filter(value__gte=1000)
                invoices = Invoices.objects.filter(state=1).filter(settle_date__gte=filter_7day).filter(value__gte=1000).filter(r_hash__in=payments.values_list('payment_hash'))
                ...
                channels_df['max_suggestion'] = channels_df.apply(lambda row: int((row['rebal_ppm']+row['profit_margin'] if row['out_rate'] > 0 else row['local_fee_rate'])*1.15) if row['in_percent'] > 75 else int(row['local_fee_rate']), axis=1)
                channels_df['max_suggestion'] = channels_df.apply(lambda row: row['local_fee_rate']*1.21 if row['max_suggestion'] > (row['local_fee_rate']*1.21) or row['max_suggestion'] == 0 else row['max_suggestion'], axis=1)
                channels_df['min_suggestion'] = channels_df.apply(lambda row: int((row['rebal_ppm'] if row['out_rate'] > 0 else row['local_fee_rate'])*0.79) if row['out_percent'] > 25 else int(row['local_fee_rate']), axis=1)
                channels_df['min_suggestion'] = channels_df.apply(lambda row: row['local_fee_rate']*0.79 if row['min_suggestion'] < (row['local_fee_rate']*0.79) else row['min_suggestion'], axis=1)
                ...
                channels_df['adjusted_out_rate'] = channels_df.apply(lambda row: int(row['out_rate']*(1+row['net_routed_7day']*row['assisted_ratio']*multiplier/100)), axis=1)
                ...
                channels_df['new_rate'] = channels_df.apply(lambda row: row['local_fee_rate']*0.98 if row['adjustment']==0 and row['out_percent']>=25 and row['net_routed_7day']==0 else row['new_rate'], axis=1)
                channels_df['new_rate'] = channels_df.apply(lambda row: row['local_fee_rate']*1.21 if row['adjustment']==0 and row['out_percent']<25 and row['failed_out_1day']>failed_htlc_limit else row['new_rate'], axis=1)
                ...
                if not update_df.empty:
                    print (update_df[['chan_id', 'in_percent', 'out_percent', 'net_routed_7day', 'revenue_7day', 'revenue_assist_7day', 'local_fee_rate', 'out_rate', 'rebal_ppm', 'profit_margin', 'max_suggestion', 'min_suggestion', 'assisted_ratio', 'adjusted_out_rate', 'adjusted_rebal_rate', 'out_rate_only', 'fee_rate_only', 'new_rate', 'adjustment']].to_string())
